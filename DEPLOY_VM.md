...
## 3. Статика и логотипы

- Чтобы логотипы и прочая статика нормально прогружались после `git pull`, на VM нужно собрать статику в каталог, откуда её отдаёт веб-сервер (nginx/apache):
  ```bash
  cd /path/to/project   # каталог проекта на VM (где manage.py)
  python manage.py collectstatic --noinput
  ```
  После этого перезапустите приложение (gunicorn) и при необходимости веб-сервер (nginx).

- Убедитесь, что в репозитории есть файлы логотипов (например, в каталоге `static/` или `images/` в корне проекта): `logo.png`, `plk_logo.png`. Если их нет в git — добавьте папку и файлы, закоммитьте и снова выполните на VM `git pull` и затем команду выше.
